...
also a PRIORITY LEVEL, applied strictly in this order (see LEVEL_ORDER):
    ...
    Level 9: Blank-Name Bridge      (not one of the 9 numbered rules above -
                                     a pre-dedup step that rescues rows
                                     Rules 2/4-9 could never touch because
                                     they have no usable First+Last Name to
                                     begin with; see blank_name_match())
...
Level 9 (Blank-Name Bridge) runs last, after every other level has already
locked its groups, as a final pre-dedup step - not one of the original 9
numbered rules, but a fix for a gap they all share: Rules 2, 4-9 all
require a real, compatible Name on BOTH sides to match at all, so a row
with a BLANK First or Last Name could never be folded into the right
person's record under any of them, no matter what else it plainly shares.
Level 9 bridges exactly that: a blank-name row is matched against a
fully-named row only when EVERY identifying field that's actually
populated on the blank-name row (whichever of Employee ID, Driver's
License, Passport, Phone Number, Email, or Full Address it has) agrees
with the named row - not just one of them. A row with a blank name plus
only a Passport number merges into a named row sharing that Passport; a
row with a blank name plus BOTH a Passport and a Driver's License number
only merges into a named row sharing BOTH (see blank_name_match()). A
blank-name row with no populated identifying field at all has nothing to
search by and is simply left as its own row. SSN/DOB compatibility and
the standard locked-group protection still apply here exactly as they do
...
def is_blank_name(r: Rec) -> bool:
    """True if this row doesn't have a usable First+Last Name - i.e. Rules
    2, 4-9 (which all require a real, compatible name on BOTH sides) could
    never match it against anything, no matter what else it shares."""
    return not (r.first and r.last)


def _blank_field_agrees(field: str, blank: Rec, other: Rec) -> bool:
    """One piece of blank_name_match()'s conjunction: does this SINGLE field
    genuinely agree between the blank-name row and the candidate named row?
    ID-type fields (Employee ID/DL/Passport/Phone/Email) agree if they share
    at least one token (parse_id_tokens() already split multi-value cells).
    Address is checked as ONE unit, the same way Rule 8 checks it: no real,
    differing sub-field (address_conflict()), and at least one sub-field
    genuinely overlapping - not just "nothing conflicts", since blank-blank
    is not evidence of anything."""
    if field == "empid":
        return not blank.empids.isdisjoint(other.empids)
    if field == "dl":
        return not blank.dl_ids.isdisjoint(other.dl_ids)
    if field == "passport":
        return not blank.passport_ids.isdisjoint(other.passport_ids)
    if field == "phone":
        return not blank.phones.isdisjoint(other.phones)
    if field == "email":
        return not blank.emails.isdisjoint(other.emails)
    if field == "address":
        if address_conflict(blank, other):
            return False
        street_overlap = bool(blank.addr) and bool(other.addr)
        other_overlap = any(a and b and a == b for a, b in zip(
            (blank.city, blank.state, zip5(blank.zip), blank.province),
            (other.city, other.state, zip5(other.zip), other.province)))
        return street_overlap or other_overlap
    return False


def blank_name_available_fields(r: Rec) -> list:
    """Which of the 6 identifying field GROUPS (Employee ID, Driver's
    License, Passport, Phone, Email, Full Address) actually have a real
    value on this row - only these are required to agree in
    blank_name_match(); a field that's blank here simply isn't checked
    (nothing to compare it against), not treated as a free pass."""
    fields = []
    if r.empids:
        fields.append("empid")
    if r.dl_ids:
    ...
    if r.emails:
        fields.append("email")
    if r.addr or r.city or r.state or r.zip or r.province:
        fields.append("address")
    return fields


def blank_name_match(r1: Rec, r2: Rec) -> bool:
    """Pre-dedup bridging step (Level 9, weakest/last priority) - not one of
    the original 9 numbered rules, since it exists purely to rescue rows
    Rules 2/4-9 could never touch in the first place: a row with a BLANK
    First or Last Name fails name_compat_match() against everything, so no
    amount of shared Employee ID/DL/Passport/Phone/Email/Address alone could
    ever fold it into the right person's record under the rules above.

    Exactly one side must be blank-name and the other a fully-named row
    (two blank-name rows, or two named rows, are not this rule's concern -
    named-named is already covered by Rules 1-8, and blank-blank has no
    name to anchor a merge to either way). EVERY field group that's
    actually populated on the blank-name side (see
    blank_name_available_fields()) must agree with the named side - not
    just one of them: a row with blank name + Passport + DL only merges
    into a named row sharing BOTH that Passport AND that DL; a row with
    blank name + Passport only merges on Passport alone. A blank-name row
    with NO populated identifying field has nothing to search by and is
    left as-is (blank_name_available_fields() returns empty, so this
    always returns False for it). SSN/DOB must still be compatible
    (matching or blank on either side) - a real, differing SSN or DOB
    still blocks the merge here too, same as every other rule."""
    r1_blank, r2_blank = is_blank_name(r1), is_blank_name(r2)
    if r1_blank == r2_blank:
        return False   # both blank or both named - not this rule's concern
    blank, named = (r1, r2) if r1_blank else (r2, r1)
    fields = blank_name_available_fields(blank)
    if not fields:
        return False
    if not (compatible(blank.ssn, named.ssn) and compatible(blank.dob, named.dob)):
        return False
    return all(_blank_field_agrees(f, blank, named) for f in fields)
...
LEVEL_BLANK_NAME = 9   # Blank-Name Bridge - pre-dedup step, weakest/last priority (see blank_name_match())
...
LEVEL_NAMES = {
    ...
    LEVEL_BLANK_NAME: "Blank-Name Bridge",
}
...
LEVEL_MATCH_FUNCS = {
    ...
    LEVEL_BLANK_NAME: blank_name_match,
}
...
def describe_bucket_key(key) -> str:
    ...
    if level == LEVEL_BLANK_NAME:
        field_label = {
            "empid": "Employee ID", "dl": "Driver's License", "passport": "Passport",
            "phone": "Phone Number", "email": "Email", "addr": "Street Address",
            "city": "City", "zip": "Zip",
        }.get(key[1], key[1])
        return f"Blank-Name Bridge ({field_label})"
    if level == LEVEL_SSN:
        return "SSN"
...
def bucket_candidate_pairs(recs):
    ...
    # Level 9 (Blank-Name Bridge) needs its OWN buckets, separate from the
    # ones above - Levels 3-8 only ever bucket a row if it already has a
    # real First+Last Name (a blank-name row could never match Rules 4-9 in
    # the first place, which is exactly the gap blank_name_match() exists to
    # close). These buckets include EVERY row - blank-name AND named -
    # sharing a token/field, but candidate pairs are only ever generated as
    # (blank-name row, named row): never named-named (already fully covered
    # by Levels 1-8 above) or blank-blank (no name on either side to anchor
    # a merge to). So a bucket holding thousands of named rows and only a
    # handful of blank-name ones costs #blank x #named pairs, not the full
    # quadratic combinations() the other levels use - the same hotspot risk
    # Level 8's own bucketing refinement guards against, just handled by
    # construction here since blank-name rows have no Last Name to further
    # split a bucket by.
    blank_name_buckets = defaultdict(list)
    for r in recs:
        for tok in r.empids:
            blank_name_buckets[(LEVEL_BLANK_NAME, "empid", tok)].append(r.idx)
        for tok in r.dl_ids:
            blank_name_buckets[(LEVEL_BLANK_NAME, "dl", tok)].append(r.idx)
        for tok in r.passport_ids:
            blank_name_buckets[(LEVEL_BLANK_NAME, "passport", tok)].append(r.idx)
        for tok in r.phones:
            blank_name_buckets[(LEVEL_BLANK_NAME, "phone", tok)].append(r.idx)
        for tok in r.emails:
            blank_name_buckets[(LEVEL_BLANK_NAME, "email", tok)].append(r.idx)
        if r.addr:
            blank_name_buckets[(LEVEL_BLANK_NAME, "addr", r.addr)].append(r.idx)
        if r.city:
            blank_name_buckets[(LEVEL_BLANK_NAME, "city", r.city)].append(r.idx)
        if r.zip:
            blank_name_buckets[(LEVEL_BLANK_NAME, "zip", r.zip)].append(r.idx)

    total_blank_buckets = len(blank_name_buckets)
    for n, (key, idxs) in enumerate(blank_name_buckets.items(), 1):
        progress("Bucketing (Blank-Name Bridge)", n, total_blank_buckets)
        if len(idxs) < 2:
            continue
        blank_idxs = [i for i in idxs if is_blank_name(recs[i])]
        named_idxs = [i for i in idxs if not is_blank_name(recs[i])]
        if not blank_idxs or not named_idxs:
            continue   # nothing to bridge in this bucket - skip entirely
        biggest_buckets.append((len(blank_idxs) + len(named_idxs), key))
        if len(biggest_buckets) > 5:
            biggest_buckets.sort(key=lambda t: -t[0])
            del biggest_buckets[5:]
        level_pair_lists[LEVEL_BLANK_NAME].extend(
            (min(a, b), max(a, b)) for a, b in itertools.product(blank_idxs, named_idxs))
