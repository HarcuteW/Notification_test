...
def main() -> None:
    ...
    for n, group_idxs in enumerate(groups, 1):
        ...
        if len(group_idxs) == 1:
            ...
            # dob_merge_fast() reuses rec.dob_primary/dob_alt (already
            # resolved in build_records()) instead of re-parsing.
            row[COL_DOB] = dob_merge_fast([rv[dob_pos], rv[dob_alt_pos]],
                                           [rec.dob_primary, rec.dob_alt])
        ...
        # dob_merge_fast() reuses each row's already-resolved
        # dob_primary/dob_alt instead of re-parsing raw cell text.
        row[COL_DOB] = dob_merge_fast(
            [r[dob_pos] for r in sub_rows] + [r[dob_alt_pos] for r in sub_rows],
            [rec.dob_primary for rec in sub_recs] + [rec.dob_alt for rec in sub_recs])
