...
8 priority levels:
  ...
  PHASE 2 - Name ID Cascade (Step 3 of the new spec)
    ...
    Two source columns are handled specially, outside the 6-level cascade:
...
# ------------------------------------------------------------
class Rec:
    __slots__ = ("idx", "first", "last", "mid", "suffix", "dob",
                 "dob_primary", "dob_alt", "ssn",
...
def build_records(df: pd.DataFrame):
    ...
    values = df.values
    fi, la, mi, sf, do, da, ss, dl, pp, tx, gv, ad, ci, st, zp, pv = (
    ...
    for i in range(len(df)):
        ...
        # Stored separately (not just collapsed into r.dob) so Step 4's
        # output build can reuse these already-resolved keys directly -
        # see dob_merge_fast() - instead of re-parsing the same raw DOB/
        # DOBs cell text a second time via norm_dob()/pd.to_datetime(),
        # which profiling showed as the single biggest hotspot in the
        # whole pipeline (dob_merge() is called once per output group).
        r.dob_primary = primary_dob
        r.dob_alt = alt_dob
...
def dob_merge_fast(raw_values, precomputed_keys) -> str:
    ...
    precomputed_keys (build_records()'s already-resolved r.dob_primary/
    r.dob_alt for that exact cell - see Rec) instead of re-parsing it via
...
def main() -> None:
    ...
    for n, group_idxs in enumerate(groups, 1):
        ...
        if len(group_idxs) == 1:
            ...
            # Uses ONLY whichever of the two this row's own r.dob actually
            # resolved from (primary-if-present-else-alt, same precedence as
            # matching) - never both, so a single row's primary and
            # alternate DOB source are never shown as if they were two
            # different real dates for the same person (see
            # _split_groups_by_dob() for the equivalent guard across rows).
            dob_raw = rv[dob_pos] if rec.dob_primary else rv[dob_alt_pos]
            row[COL_DOB] = dob_merge_fast([dob_raw], [rec.dob])
        ...
        # COL_DOB_ALT ("DOBs") feeds the same date evidence as COL_DOB - see
        # module docstring. Each row contributes ONLY whichever of the two
        # its own r.dob actually resolved from (primary-if-present-else-alt,
        # same precedence as matching) - never both - so one row's primary
        # and alternate DOB source can never look like two different real
        # dates for the same person. By the time a group reaches this point,
        # _split_groups_by_dob() has already guaranteed every row here
        # shares the same (or a blank) r.dob, so this should always collapse
        # to at most one displayed date - never a semicolon-joined list of
        # genuinely different real dates.
        dob_raws = [r[dob_pos] if rec.dob_primary else r[dob_alt_pos]
                    for r, rec in zip(sub_rows, sub_recs)]
        row[COL_DOB] = dob_merge_fast(dob_raws, [rec.dob for rec in sub_recs])
