...
After the 8 priority levels finish and every group is built into one output
row per confirmed person, a separate POST-BUILD pass - the Unknown Name
Bridge - runs before the DOCID-overflow split (see below): any output row
whose First AND Last Name are both still blank (no real name was found
anywhere in that merged group) is first backfilled to display "[UNKNOWN]"
in both columns, then compared against every row that DOES have a real
name in either column. A match is only made on SSN first, and - only when
...
# Display placeholder backfilled into a still-blank First/Last Name output
# cell, once every merge group is already built (see backfill_unknown_names()
# in main()). "UNKNOWN" is already in NAME_PLACEHOLDERS above, so this value
# is itself always treated as blank/non-conflicting by every name-matching
# function - it never accidentally becomes real name evidence.
...
# ------------------------------------------------------------
# 7b) Unknown Name Bridge - a POST-BUILD pass (see main()), run once every
#     group from the 8 priority levels has already become one output row,
#     and BEFORE the DOCID-overflow split. Folds a row with NO real name at
#     all into the single best-matching named row, using only SSN, then (only
#     if no SSN match exists) Employee ID/DL/Passport/Email - never Name
#     (there is none) and never Address alone.
# ------------------------------------------------------------
def is_row_blank_name(row: dict) -> bool:
    """True once an output row's First AND Last Name have both already been
    backfilled to NAME_UNKNOWN (see backfill_unknown_names()) - i.e. no real
    name was found anywhere in that merged group. Used to split the built
    rows into the named set (candidates to bridge INTO) and the
    fully-unknown set (candidates to bridge FROM) in
    bridge_unknown_name_rows()."""
    return row[COL_FIRST] == NAME_UNKNOWN and row[COL_LAST] == NAME_UNKNOWN


def backfill_unknown_names(out_rows: list) -> None:
    """In place: any output row whose First or Last Name ended up blank
    (fullest_value() found no real name anywhere in that merged group) is
    backfilled to the display placeholder NAME_UNKNOWN, so the output never
    shows a literally empty name cell. Must run AFTER every group is already
    built (a row counts as 'blank' only once the WHOLE group has been
    checked, not just one source row) and BEFORE bridge_unknown_name_rows(),
    which relies on NAME_UNKNOWN to tell a still-unidentified row apart from
    a genuinely named one."""
    for row in out_rows:
        if not row[COL_FIRST]:
            row[COL_FIRST] = NAME_UNKNOWN
        if not row[COL_LAST]:
            row[COL_LAST] = NAME_UNKNOWN


def _row_dob_set(row: dict) -> frozenset:
    """Every distinct real date already in an output row's DOB cell (a "; "-
    joined list - see dob_merge()), re-parsed via norm_dob() for comparison.
    A legitimately-merged row should never actually hold 2+ conflicting real
    dates (the main clustering pass already refuses to merge 2 different
    real DOBs together - see group_dob/try_union() in main()), but this
    doesn't assume that; it just compares whichever dates are present."""
    return frozenset(norm_dob(tok) for tok in row[COL_DOB].split(MERGE_SEP) if norm_dob(tok))


def _row_id_tokens(row: dict, col: str) -> frozenset:
    """An output row's already-merged Employee ID/DL/Passport/Email cell,
    split back into individual tokens (parse_id_tokens() already handles the
    "; "-joined format these columns are built in)."""
    return parse_id_tokens(row[col])


def _merge_name_field(base_val: str, extra_val: str) -> str:
    """Picks the fullest real name value between a named row's field and the
    unknown row's field being folded into it - but falls back to the named
    row's OWN existing value (never to "") if neither side has a real,
    non-placeholder value. Without this fallback, folding an "[UNKNOWN]"
    Middle Name/Suffix into a named row whose own Middle Name/Suffix is also
    blank would erase that field back to blank text, undoing
    backfill_unknown_names() for First/Last Name specifically."""
    merged = fullest_value([base_val, extra_val], [norm_name(base_val), norm_name(extra_val)])
    return merged or base_val


def _merge_unknown_into_named(base: dict, extra: dict) -> None:
    """Folds one fully-unknown-name output row ('extra') into its matched
    named output row ('base'), in place - called only from
    bridge_unknown_name_rows() once a match is already decided.

    DOCID and every OTHER_MERGE_COLS column are combined exactly the way the
    main clustering pass already combines them for any group - plain
    semicolon_merge(). Address fields (ADDRESS_COLS + 'Other Address') are
    ALSO just semicolon_merge()'d here as plain text - unlike the main
    path's split_addresses() (majority address vs. 'Other Address'
    clustering) - because an unknown-name row was never confirmed to share
    the SAME physical address as 'base', only some other identifier
    (SSN/Employee ID/DL/Passport/Email); appending it as extra evidence
    rather than letting it compete to become the 'majority' address is the
    safer choice for this bridge specifically."""
    for col in [COL_DOCID] + OTHER_MERGE_COLS + ADDRESS_COLS + ["Other Address"]:
        base[col] = semicolon_merge([base.get(col, ""), extra.get(col, "")])
    base[COL_DOB] = dob_merge([base[COL_DOB], extra[COL_DOB]])
    base[COL_FIRST] = _merge_name_field(base[COL_FIRST], extra[COL_FIRST])
    base[COL_MIDDLE] = _merge_name_field(base[COL_MIDDLE], extra[COL_MIDDLE])
    base[COL_LAST] = _merge_name_field(base[COL_LAST], extra[COL_LAST])
    base[COL_SUFFIX] = _merge_name_field(base[COL_SUFFIX], extra[COL_SUFFIX])
...
def bridge_unknown_name_rows(out_rows: list) -> tuple:
    """POST-BUILD pass (see main()) - every output row with NO real name at
    all (both First and Last already backfilled to NAME_UNKNOWN) is folded
    into the single BEST-matching named row, using only the identifiers
    below - never Name (there is none) and never Address alone (Address is
    appended once a match is already found some other way, never used to
    decide it - a shared address alone is not enough evidence for who an
    unnamed record belongs to).

    Two-tier cascade:
      Tier 1 - SSN: the unknown row's SSN exactly matches a named row's SSN.
      Tier 2 - only checked when NO Tier-1 candidate exists at all: the
               unknown row shares at least one Employee ID, Driver's
               License, Passport, or Email token with a named row.
    Within whichever tier actually has candidates, the named row sharing the
    MOST such identifiers wins. A genuinely conflicting DOB (both known,
    ...
    if not set2_idx or not set1_idx:
        return out_rows, []

    ID_COLS = (COL_EMPID, COL_DL, COL_PASSPORT, COL_EMAIL)
    set1_profiles = [{
        "ssn": norm_ssn(out_rows[i][COL_SSN]),
        "dob": _row_dob_set(out_rows[i]),
        "ids": {c: _row_id_tokens(out_rows[i], c) for c in ID_COLS},
    } for i in set1_idx]

    # Blocking indexes - built ONCE, so an unknown row never has to test
    # itself against every named row (that would be O(unknown x named),
    # quadratic on a large file). Instead it only ever tests the small set
    # of named rows that already share an exact SSN or ID token with it -
    # the same "only compare what already matches something exact" approach
    # the main clustering pass (bucket_candidate_pairs()) uses.
    ssn_index = defaultdict(list)     # ssn pattern -> [position in set1_profiles, ...]
    id_index = defaultdict(list)      # (id column, token) -> [position in set1_profiles, ...]
    for pos, prof in enumerate(set1_profiles):
        if prof["ssn"]:
            ssn_index[prof["ssn"]].append(pos)
        for c in ID_COLS:
    ...
    for i2 in set2_idx:
        r2 = out_rows[i2]
        ssn2 = norm_ssn(r2[COL_SSN])
        dob2 = _row_dob_set(r2)
        ids2 = {c: _row_id_tokens(r2, c) for c in ID_COLS}

        def _survives(pos):
            """None if this candidate is disqualified by a genuinely
            conflicting DOB or SSN; otherwise its id_match count (used only
            for the same-tier tie-break)."""
            prof = set1_profiles[pos]
            if prof["dob"] and dob2 and prof["dob"].isdisjoint(dob2):
                return None   # real, differing DOB - disqualified regardless of anything else
            if ssn2 and prof["ssn"] and ssn2 != prof["ssn"]:
                return None   # real, differing SSN - disqualified regardless of anything else
            return sum(
                1 for c in ID_COLS
                if ids2[c] and prof["ids"][c] and not ids2[c].isdisjoint(prof["ids"][c])
            )

        # Tier 1 (SSN): only candidates sharing the exact SSN, via ssn_index -
        # never the full named set. Falls through to Tier 2 if NONE of them
        # survive the conflict check above (e.g. the only SSN match has a
        # genuinely conflicting DOB) - not just when ssn2 itself is blank.
        tier1_pool = []
        if ssn2:
            for pos in ssn_index.get(ssn2, []):
                m = _survives(pos)
                if m is not None:
                    tier1_pool.append((pos, m))

        if tier1_pool:
            pool, tier_label = tier1_pool, "SSN"
        else:
            seen = set()
            pool = []
            for c in ID_COLS:
                for tok in ids2[c]:
                    for pos in id_index.get((c, tok), []):
                        if pos in seen:
                            continue
                        seen.add(pos)
                        m = _survives(pos)
                        if m is not None and m > 0:
                            pool.append((pos, m))
            tier_label = "Employee ID/Driver's License/Passport/Email"
        if not pool:
            continue   # nothing survives at either tier - leave this row standing alone
    ...
    for i, row in enumerate(out_rows):
        if i in consumed:
            continue
        for i2 in absorbed_into.get(i, []):
            _merge_unknown_into_named(row, out_rows[i2])
...
def main() -> None:
    ...
    for n, group_idxs in enumerate(groups, 1):
        ...
        if len(group_idxs) == 1:
            ...
            # DOCID overflow (splitting into 'DOCIDs 2', 'DOCIDs 3', ...)
            # deliberately does NOT happen here anymore - it now runs once,
            # on the FINAL row set, after the Unknown Name Bridge pass below
            # (see main()), so a DOCID list gained via bridging is chunked
            # correctly instead of being split before it's even complete.
        ...
        # DOCID overflow split now runs once, on the FINAL row set, after the
        # Unknown Name Bridge pass below (see main()) - not here - so a
        # DOCID list gained via bridging is chunked correctly.
    ...
    t0 = time.monotonic()
    backfill_unknown_names(out_rows)
    n_before_bridge = len(out_rows)
    out_rows, unknown_bridge_review = bridge_unknown_name_rows(out_rows)
    n_bridged = n_before_bridge - len(out_rows)
    print(f"  Done. ({time.monotonic() - t0:.1f}s)")
    if n_bridged:
        print(f"  {n_bridged:,} fully-unknown-name row(s) folded into a named row "
              f"(SSN, then Employee ID/DL/Passport/Email).")
    if unknown_bridge_review:
        print(f"  {len(unknown_bridge_review):,} fully-unknown-name row(s) had an "
              f"ambiguous best match (2+ equally-strong named-row candidates) and were "
              f"left unmerged - see the 'Unknown Name Bridge Review' sheet.")

    # DOCID overflow split runs here, once, on the FINAL row set - AFTER the
    # Unknown Name Bridge pass above, so a DOCID list gained via bridging is
    # split correctly instead of being chunked before it was even complete.
    ...
    df_unknown_bridge_review = pd.DataFrame(unknown_bridge_review)
    ...
    _write_workbook(OUTPUT_XLSX, {
        ...
        "Unknown Name Bridge Review": df_unknown_bridge_review,
    })
